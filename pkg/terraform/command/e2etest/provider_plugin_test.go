...
func TestProviderProtocols(t *testing.T) {
	...
	simple6ProviderExe := e2e.GoBuild("github.com/fugue/regula/pkg/terraform/provider-simple-v6/main", simple6Provider)

	simpleProvider := filepath.Join(tf.WorkDir(), "terraform-provider-simple")
	simpleProviderExe := e2e.GoBuild("github.com/fugue/regula/pkg/terraform/provider-simple/main", simpleProvider)
