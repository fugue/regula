...
package rules.cfn_aws_s3_https_access
...
test_valid_https_access_bucket_policy {
    pol = policy with input as inputs.valid_https_access_bucket_policy_infra.mock_input
    by_resource_id = {p.id: p.valid | pol[p]}
...
test_invalid_https_access_bucket_policy {
    pol = policy with input as inputs.invalid_https_access_bucket_policy_infra.mock_input
    by_resource_id = {p.id: p.valid | pol[p]}
    count(by_resource_id) == 2
...
test_invalid_missing {
    pol = policy with input as inputs.invalid_missing_infra.mock_input
    by_resource_id = {p.id: p.valid | pol[p]}
